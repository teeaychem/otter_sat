...
const FRAT_RS_PATH: &str = "./tests/frat-rs";
...
fn frat_verify(file_path: PathBuf, config: Config) -> bool {
    ...
    let listener_handle = {
        ...
        thread::spawn(|| otter_lib::io::listener::frat_receiver(rx, frat_path))
    };
...
mod frat_tests {
    ...
    fn cnf_dir() -> PathBuf {
        PathBuf::from(".").join("tests").join("cnf")
    }

    #[test]
    fn frat_setup_check() {
        let file_path = cnf_dir().join("frat").join("tt.cnf");
