...
mod basic {
    ...
    fn conflict() {
        ...
        let not_p_not_q_clause = vec![abLiteral::fresh(p, false), abLiteral::fresh(q, false)];
        assert!(the_context.add_clause(not_p_not_q_clause).is_ok());

        let p_not_q_clause = vec![abLiteral::fresh(p, true), abLiteral::fresh(q, false)];
        assert!(the_context.add_clause(p_not_q_clause).is_ok());

        let not_p_q_clause = vec![abLiteral::fresh(p, false), abLiteral::fresh(q, true)];
    ...
    fn duplicates() {
        ...
        let p_p_q_q_clause = vec![
            abLiteral::fresh(p, true),
            abLiteral::fresh(p, true),
            abLiteral::fresh(q, true),
            abLiteral::fresh(q, true),
        ];
    ...
    fn tautology_skip() {
        ...
        let p_q_not_p_clause = vec![
            abLiteral::fresh(p, true),
            abLiteral::fresh(q, false),
            abLiteral::fresh(p, false),
        ];
