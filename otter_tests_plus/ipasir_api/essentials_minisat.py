...
def test_essentials():
    essentials_test_directory = CURRENT_DIR.joinpath(
        pathlib.Path("ipasir", "app", "genipaessentials", "inputs")
    )

    for cnf in sorted(glob.glob(f"{essentials_test_directory}/*.cnf")):
        ...
        otter_sat = subprocess.run(
            ["./genipaessentials_otter_sat", cnf],
            cwd=BUILD_DIR,
            capture_output=True,
            text=True,
        )
