    ...
    for row in 1..GRID_SIZE + 1 {
        for col in 1..GRID_SIZE + 1 {
            ...
            for value in 1..GRID_SIZE + 1 {
                cell_restriction_clause.push(abLiteral::fresh(
                ...
                for other_value in 1..GRID_SIZE + 1 {
                    if other_value != value {
                        let exclusion_clause = vec![
                            abLiteral::fresh(cell_atom(row, col, value, context, cell_map), false),
                            abLiteral::fresh(
...
fn add_clauses_each_row_has_all_values(
    ...
    for value in 1..GRID_SIZE + 1 {
        for row in 1..GRID_SIZE + 1 {
            ...
            for col in 1..GRID_SIZE + 1 {
                row_value_clause.push(abLiteral::fresh(
                    cell_atom(row, col, value, context, cell_map),
                    true,
                ));
            }
...
fn add_clauses_each_col_has_all_values(
    ...
    for value in 1..GRID_SIZE + 1 {
        for col in 1..GRID_SIZE + 1 {
            ...
            for row in 1..GRID_SIZE + 1 {
                col_value_clause.push(abLiteral::fresh(
                    cell_atom(row, col, value, context, cell_map),
                    true,
                ));
            }
            ...
            for sub_grid_c in 0..GRID_SIZE / 3 {
                ...
                for row in 1..(GRID_SIZE / 3) + 1 {
                    for col in 1..(GRID_SIZE / 3) + 1 {
                        subgrid_val_clause.push(abLiteral::fresh(
                            cell_atom(
                                row + (sub_grid_r * 3),
                                col + (sub_grid_c * 3),
                                value,
...
fn add_clauses_detailing_puzzle(
    ...
    for row in 0..GRID_SIZE {
        for col in 0..GRID_SIZE {
            ...
            if value != 0 {
                let clause =
                    abLiteral::fresh(cell_atom(row + 1, col + 1, value, context, cell_map), true);
