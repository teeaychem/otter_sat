...
#[derive(Default)]
pub struct Watches {
    /// A mapping of [Atom](crate::structures::atom)s to [WatchDB]s, implicitly given by interpreting each index as an atom.
    pub dbs: Vec<WatchDB>,
}
...
impl Watches {
    ...
    ///
    /// The counterpart of [unwatch_long](Watches::unwatch_long).
    pub fn watch_long(&mut self, literal: &CLiteral, watch: LongWatch) {
        // # Safety: A [WatchDB] is always constructed for an atom.
        let atom = unsafe { self.dbs.get_unchecked_mut(literal.atom() as usize) };
    ...
    ///
    /// The counterpart of [watch_long](Watches::watch_long).
    /*
    ...
    pub fn binary_watched(&self, literal: &CLiteral) -> *const Vec<BinaryWatch> {
        // # Safety: A [WatchDB] is always constructed for an atom.
    ...
    pub fn long_watched(&mut self, literal: &CLiteral) -> *mut Vec<LongWatch> {
        // # Safety: A [WatchDB] is always constructed for an atom.
        let atom = unsafe { self.dbs.get_unchecked_mut(literal.atom() as usize) };
